...
class HelpSystem:
    ...
    def show_installation_help(ctx, parent_frame):
        ...
        help_text = _(
            ...
            "LibreOffice runs TejOCR inside its own Python runtime, so install Python packages there, not in your system Python.\n\n"
            "Installation steps:\n"
            ...
            "4) Recommended preprocessing package in LibreOffice Python:\n"
            f"   `{_runtime_pip_install_command(['pillow'])}`\n"
            "5) Optional compatibility packages in LibreOffice Python:\n"
            f"   `{_runtime_pip_install_command(['numpy', 'pytesseract'])}`\n"
            "6) PDF files are supported in file-based OCR. Install one of:\n"
            ...
            "7) Keep your Tesseract language packs up to date.\n\n"
            "More reference:\n"
    ...
    def show_usage_help(ctx, parent_frame):
        ...
        help_text = _(
            ...
            "4) Click Start OCR\n\n"
            "OCR from file:\n"
            "1) Choose OCR Image/PDF from File from the TejOCR top menu\n"
            "2) Pick one or more files (images and/or PDFs)\n"
            "3) For PDFs, TejOCR extracts text page by page\n"
            "4) Select language and output destination\n"
            "5) Click Start OCR\n\n"
            "Batch behavior (file flow):\n"
            "- Select multiple images/PDFs in one run.\n"
            "- Merge Batch Results merges recognized output into one block with source headers.\n"
